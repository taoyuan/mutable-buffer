...
function itWriteFixedNumber(type: string, val: any, expected: any) {
  ...
  it(fn + '(' + val + ')', function () {
    ...
    const result = (<any>buffer)[fn](val).join();
...
function itWriteLengthNumber(
  ...
  it(fn + '(' + val + ')', function () {
    const buffer = new MutableBuffer();
    const result = (<any>buffer)[fn](val, length).join();
    assertBuffers(result, expected, fn);
  });
}
...
describe('mutable-buffer', function () {
  describe('writing number', function () {
    ...
    it('writing multiple number', function () {
      ...
      expect(buffer.size).equal(12);
      assertBuffers(buffer.join(), [0, 0, 0, 1, 0, 0, 0, 0x0a, 0, 0, 0, 0]);
    });
  });
  ...
  describe('having to resize the buffer', function () {
    it('after resize correct result returned', function () {
      ...
      expect(buffer.size).equal(12);
      expect(buffer.capacity()).equal(20);
      assertBuffers(buffer.join(), [0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0, 1]);
    });
  });
  ...
  describe('clearing', function () {
    ...
    it('clears', function () {
      buffer.clear();
      assertBuffers(buffer.join(), []);
    });
    it('writing more', function () {
      const joinedResult = buffer
        .writeCString('!')
        .writeInt32BE(9)
        .writeInt16BE(2)
        .join();
